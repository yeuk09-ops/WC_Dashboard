...
**당신은 전체 법인 데이터를 받았습니다. 반드시 다음 프로세스를 따르세요:**

**STEP 1: 전체 법인 이슈 점수화 (필수!)**
데이터에 포함된 모든 법인(국내, 중국, 홍콩, ST, 기타)의 모든 문제 이슈를 점수화하세요:
- 국내 재고: 증감률 +XX%, DIO +YY일 → 점수: ZZ점
- 중국 재고: 증감률 +XX%, DIO +YY일 → 점수: ZZ점
- 중국 매출채권: 증감률 +XX%, DSO +YY일 → 점수: ZZ점
- 홍콩 재고: 증감률 +XX%, DIO +YY일 → 점수: ZZ점
- ST 재고: 증감률 +XX%, DIO +YY일 → 점수: ZZ점
(계속...)
...
1위. 중국 재고: 95점 → HIGH
2위. ST 재고: 85점 → HIGH
3위. 중국 매출채권: 60점 → MEDIUM
4위. 국내 재고: 50점 → MEDIUM
5위. 홍콩 재고: 25점 → LOW

**STEP 3: 필터링 및 출력**
${entity === '연결' 
   ? '**상위 2-3개만 선택하여 액션플랜에 포함**'
   : `**${entity} 법인의 이슈만 필터링하되, 우선순위는 STEP 2의 전체 순위 그대로 사용**
   
   예시: 국내(OC) 법인 선택 시
   - 전체 4위인 "국내 재고(50점)" → priority: "MEDIUM"
   - 전체 순위가 MEDIUM이므로 국내 법인 액션플랜에도 MEDIUM으로 표시`
}

**전체 법인 기준 내부 점수화 예시 (점수는 응답에 포함하지 않음):**

STEP 1: 각 이슈 점수 계산
- 중국 재고: 485.7억 증가
  * 증감률 +51% (50점) + DIO +39일 (25점) + 비중 35% (20점) = 95점
- ST 재고: 73.3억 증가
  * 증감률 +142% (50점) + DIO +258일 (30점) + 비중 3% (5점) = 85점
- 중국 매출채권: 156.7억 증가
  * 증감률 +19% (30점) + DSO +2일 (5점) + 비중 35% (20점) = 55점
- 국내 재고: 50.2억 증가
  * 증감률 +2% (10점) + DIO +27일 (20점) + 비중 35% (20점) = 50점
- 홍콩 재고: 13.1억 증가
  * 증감률 +9% (20점) + DIO +1일 (5점) + 비중 4% (0점) = 25점
...
${entity !== '연결' 
  ? `
**"${entity}" 법인 특별 지침:**
- 위 전체 법인 점수화 결과에서 **${entity} 법인의 이슈만** 추출
- 우선순위(HIGH/MEDIUM/LOW)는 전체 법인 기준 그대로 유지
- 예: 국내 재고가 전체 기준으로 MEDIUM이면 → 국내 법인 액션플랜에도 MEDIUM으로 표시
- ${actionCountGuide}
`
  : ''
}
...
**🚨 우선순위 결정 프로세스 (다시 한번 강조!):**
${entity === '연결' 
  ? `
1. 전체 법인(국내, 중국, 홍콩, ST, 기타)의 모든 문제 이슈를 점수화
2. 점수 순으로 정렬
3. 상위 2-3개만 선택
4. 선택된 항목의 우선순위 결정 (80점 이상 HIGH, 50-79 MEDIUM, 50 미만 LOW)
`
  : `
1. **전체 법인(국내, 중국, 홍콩, ST, 기타)의 모든 문제 이슈를 점수화**
   - ${entity} 법인만이 아닌 전체 법인!
2. **점수 순으로 정렬**
3. **${entity} 법인의 이슈만 필터링**
4. **우선순위는 전체 법인 기준 유지**
   - 예: ${entity} 재고가 전체 1위(95점) → HIGH
   - 예: ${entity} 매출채권이 전체 3위(60점) → MEDIUM
`
}
