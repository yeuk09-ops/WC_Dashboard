...
**우선순위 결정 프로세스:**
1. **전체 법인(국내, 중국, 홍콩, ST, 기타)의 모든 주요 이슈를 내부적으로 점수화**
2. **전체 이슈를 점수 순으로 정렬**
3. **점수 기준으로 HIGH/MEDIUM/LOW 결정**
4. ${entity === '연결' 
   ? '**상위 2-3개만 선택하여 액션플랜에 포함**'
   : `**${entity} 법인의 이슈만 필터링하되, 우선순위는 전체 기준 유지**`
}

**전체 법인 기준 내부 점수화 예시 (점수는 응답에 포함하지 않음):**
- 중국 재고: 485.7억 증가, 증감률 +51%, DIO +39일, 비중 35% → **(내부 점수: 95점) → HIGH**
- ST 재고: 73.3억 증가, 증감률 +142%, DIO +258일, 비중 3% → **(내부 점수: 85점) → HIGH**  
- 중국 매출채권: 156.7억 증가, 증감률 +19%, DSO +2일, 비중 35% → **(내부 점수: 60점) → MEDIUM**
- 국내 재고: 50.2억 증가, 증감률 +2%, DIO +27일, 비중 35% → **(내부 점수: 50점) → MEDIUM**
- 홍콩 재고: 13.1억 증가, 증감률 +9%, DIO +1일, 비중 4% → **(내부 점수: 25점) → LOW**
...
```json
{
  "improvementDirection": "중국 법인 개선 방향...",
  "actionItems": [
    {
      "priority": "HIGH",
      "label": "재고",
      "issue": "중국 법인 재고 전년동기 대비 485.7억원 증가...",
      "action": "...",
      "target": "약 200억원 자금 회수 가능",
      "responsible": "중국 법인 재무팀"
    },
    {
      "priority": "MEDIUM",
...
```
→ 중국 재고: HIGH (전체 1위, 95점)
→ 중국 매출채권: MEDIUM (전체 3위, 60점)
